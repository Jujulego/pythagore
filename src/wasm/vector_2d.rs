...
pub struct Vector2D(Vector2<f64>);
...
impl Vector2D {
    ...
    pub fn new(dx: f64, dy: f64) -> Vector2D {
    ...
    pub fn dot_scalar(&self, scalar: f64) -> Vector2D {
        Vector2D(self.0 * scalar)
    }

    pub fn div_scalar(&self, scalar: f64) -> Vector2D {
        Vector2D(self.0 / scalar)
    }
...
// Conversions
impl AsRef<Vector2<f64>> for Vector2D {
    fn as_ref(&self) -> &Vector2<f64> {
        &self.0
    }
}

impl AsMut<Vector2<f64>> for Vector2D {
